...
GLuint objloader::load(const QString &filename)
{
    ...
    for (size_t i = 0; i < N_coord; ++i)
    {
        if(coord[i][0] != '#')
        {
            if (coord[i][0] == 'v' && coord[i][1] == ' ') // vertex
            {
            ...
            } else if (coord[i][0] =='v' && coord[i][1]=='t') // texture vertex
            {
                float tmpx,tmpy;
                sscanf(coord[i].c_str(),"vt %f %f",&tmpx,&tmpy);
                uvs.push_back(QVector2D(tmpx,tmpy));
            } else if(coord[i][0] == 'f')     // edge
            {
...
GLuint objloader::draw(GLfloat offset)
{
    ...
    for(size_t i=0; i < N_faces; ++i)
    {
        ...
        // building a texture vertex along the vertex of the normal, gives another texture mapping to the model
    ...
    glEndList();
    return num;     // return number of display list
}
